...
# נתיב Deposit (כחלק מהלוגיקה העסקית של Core)
...
# נתיב Withdraw (כחלק מהלוגיקה העסקית של Core)
@app.route('/withdraw', methods=['POST'])
def withdraw():
    start_time = time.time()
    # לוגיקת עיבוד עבור Withdraw
    ...
    WITHDRAW_DURATION.observe(time.time() - start_time)
...
if __name__ == '__main__':
    start_http_server(8001)  # תחילת שרת Prometheus
